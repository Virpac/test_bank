...
from src.main.api.foudation.http_requester import HttpRequester
...
class ValidateCrudRequester(HttpRequester):
    ...
    def post(self,model : Optional[BaseModel] = None) -> BaseModel:
        ...
        with allure.step(f"POST {Config.fetch("backendUrl")}{self.endpoint.value.url} and Validation Model"):
